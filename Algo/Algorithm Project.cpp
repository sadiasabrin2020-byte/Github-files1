...
/*
 Smart Event & Venue Scheduling System
 - Event: name, start (minutes), end (minutes), audience, profit, preferredVenue (optional)
 - Venue: name, capacity, availableStart, availableEnd
 - Techniques:
    * Divide & Conquer: merge sort events by end time
    * Greedy: activity selection (max count) - provided as helper
    * Dynamic Programming: Weighted Interval Scheduling (maximize profit)
    * Assignment: for each venue, choose best non-overlapping set (DP) among compatible events
*/

// --------------------------- Utilities: time helpers ---------------------------
int parseTimeToMinutes(const string &hhmm) {
    // expects "HH:MM" like "09:30" or "9:30"
    int hh, mm;
    char colon;
...
struct Event {
    ...
    int end;        // minutes from 00:00
    int audience;   // expected audience
    int profit;     // importance/profit value
    string preferredVenue; // empty if no preference
    bool assigned = false; // updated after scheduling
};
...
// --------------------------- Divide & Conquer: Merge Sort (by event end time) ---------------------------
...
// --------------------------- Helper: binary search previous compatible event ---------------------------
// Given a sorted-by-end list of indices, for index j find the largest i < j such that events[idx[i]].end <= events[idx[j]].start
...
/*
 Input: list of event indices (subset of global events) -> we assume we will sort them by end time inside
 Output: vector of selected event indices (from the passed list) that maximize sum(profit), non-overlapping
*/
vector<int> weightedIntervalScheduling(const vector<int> &subsetIdx) {
    int n = subsetIdx.size();
    if (n == 0) return {};

    // create a local vector sortedIdx that holds indices into global events, sorted by end time
    vector<int> sortedIdx = subsetIdx;
    mergeSortIdxByEnd(sortedIdx, 0, n - 1);

    // compute p[j] = index of the rightmost event i < j that is compatible, else -1
    vector<int> p(n, -1);
    for (int j = 0; j < n; ++j) {
        p[j] = findPreviousCompatible(sortedIdx, j);
    }

    // dp[j] = max profit using events [0..j]
    vector<int> dp(n, 0);
    ...
    vector<int> selected; // will store indices into global events
    int j = n - 1;
    while (j >= 0) {
        int includeProfit = events[sortedIdx[j]].profit + (p[j] != -1 ? dp[p[j]] : 0);
...
// --------------------------- Greedy: Activity selection (max count) ---------------------------
...
// --------------------------- Assignment: assign events to venues ---------------------------
/*
 Strategy:
  - Reset all events.assigned = false
  - For each venue (maybe iterate by larger capacity first to place big events first):
      - create candidate list = unassigned events that:
          * fit within venue availableStart/availableEnd
          * audience <= venue.capacity
          * if event.preferredVenue non-empty and different from this venue, deprioritize (we still allow if nothing else)
      - Run weightedIntervalScheduling on candidate list to maximize total profit for that venue
      - Mark returned events as assigned and record mapping
  - We return a mapping venueId -> vector<event indices> (final schedule)
*/
map<int, vector<int>> assignEventsToVenues() {
    // reset assigned flags
    for (auto &e : events) e.assigned = false;

    // sort venues by capacity descending (to place large events first)
    vector<int> vIdx;
    ...
    map<int, vector<int>> schedule; // venue.id -> list of event indices

    for (int vidx : vIdx) {
        Venue &v = venues[vidx];
        // build candidate events for this venue
        ...
        for (int i = 0; i < (int)events.size(); ++i) {
            ...
            // if event has preferredVenue and it's not this venue, it's still candidate but we might deprioritize
        ...
        // Run weighted interval scheduling to maximize profit for this venue
        vector<int> chosen = weightedIntervalScheduling(candidates);

        // Assign chosen events to this venue
        for (int eidx : chosen) {
